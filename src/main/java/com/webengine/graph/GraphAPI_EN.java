...
@Service
public class GraphAPI_EN {

	private final static Logger LOGGER = Logger.getLogger(GraphAPI_EN.class.getName());

	private GraphDatabaseService graphDB_en;
	...
	@PostConstruct
	void setup() {
		String db_path = graphdbfolder+"_en"; //System.getProperty("user.dir") + "/cooccsdatabase_en";
		File database = new File(db_path);
		graphDB_en  = new GraphDatabaseFactory()
		...
		//= new GraphDatabaseFactory().newEmbeddedDatabase(database);
	...
	public Vector<String> listNodes() {
		...
		try (Transaction tx = graphDB_en.beginTx()) {
			 nodelist = graphDB_en.findNodes(Labels.SINGLE_NODE);
	...
	public double getNodeDistance(String term1, String term2) {
	    ...
	    {  
			...
			Node temp = graphDB_en.findNode(Labels.SINGLE_NODE,"name", term1.toLowerCase());
			
			if (temp!=null) {
				
				Node temp2 = graphDB_en.findNode(Labels.SINGLE_NODE,"name", term2.toLowerCase());
	...
	public void addDocumentNode(String filename) {
		...
		try (Transaction tx = graphDB_en.beginTx()) {
			...
			if (f_flag == false) {
				Node docnode = graphDB_en.createNode(Labels.SINGLE_NODE);
...
//		LOGGER.info("->addNode() is started...");
		...
		Transaction tx = graphDB_en.beginTx();
		...
		try  {
			...
			// wenn es das wort schon gibt leg es nicht an
			if (f_flag == false) {
				Node wordnode = graphDB_en.createNode(Labels.SINGLE_NODE);
	...
	public void addRelation(Word word1, Word word2, int deltaSig) {
		...
		try {		
			if (!word1.getStemmedWord().equals(word2.getStemmedWord())) {
				Node n1 = graphDB_en.findNode(Labels.SINGLE_NODE, "name", word1.getStemmedWord().toLowerCase());
				Node n2 = graphDB_en.findNode(Labels.SINGLE_NODE, "name", word2.getStemmedWord().toLowerCase());
				int count = 0;
	...
	//this function when called, updates the Dice ratio and costs for all the relationships present in the database
			public void updateDiceandCosts()
			{
				...
				double dice;
				ResourceIterator<Node> nodelist = null;
				Transaction tx5 = graphDB_en.beginTx();
			    ...
			    {  	
					...
					while(nodelist.hasNext())
					{
						...
						if (wordnode.hasProperty("name")) {
							...
							//LOGGER.info("Node found " +wordnode.getProperty("name"));
					        
							
							
						String node1=(String)wordnode.getProperty("name");
						countA=(int)wordnode.getProperty("occur");
						Node temp = graphDB_en.findNode(Labels.SINGLE_NODE,"name", node1.toLowerCase());
					    ...
					    //    LOGGER.info("Updating relation: " +temp.getProperty("name") + "<->" +n2.getProperty("name") + "  " + relationship.getProperty("dice") +   " " + relationship.getProperty("cost"));
			...
			public HashMap getCentroidbySpreadingActivation(Vector query)
			{
				...
				if (!query.isEmpty()) {
					...
					if (query.size()>1) {	
						...
						//LOGGER.info("database opened for querying");
						try (Transaction tx2 = graphDB_en.beginTx()) 
					    {  
						...
						//query cleaning !!!!
							...
							for (int i=0; i<query.size(); i++) {
								Node temp = graphDB_en.findNode(Labels.SINGLE_NODE,"name", query.get(i).toString().toLowerCase());
							...
							HashMap numberofreachednodes = new HashMap();
								...
								Iterator iteratorq1 = helpqueryset.iterator(); 
								   ...
								   while (iteratorq1.hasNext()){
									   ...
									   Node temp = graphDB_en.findNode(Labels.SINGLE_NODE,"name", queryterm.toLowerCase());
									   ...
									   Iterator iteratorq2 = helpqueryset.iterator(); 
										   ...
										   while (iteratorq2.hasNext()){
											   ...
											   if (!queryterm.equals(queryterm2)) {
												   Node temp2 = graphDB_en.findNode(Labels.SINGLE_NODE,"name", queryterm2.toLowerCase());
							...
							for (int k=0; k<helpquerysize; k++) {
								...
								Iterator iterator = helpqueryset.iterator(); 
								   ...
								   while (iterator.hasNext()){
									   ...
									   Node temp = graphDB_en.findNode(Labels.SINGLE_NODE,"name", queryterm);
									   ...
									   Iterator iterator2 = helpqueryset.iterator(); 
										   ...
										   while (iterator2.hasNext()){
											   ...
											   if (!queryterm.equals(queryterm2)) {
												   Node temp2 = graphDB_en.findNode(Labels.SINGLE_NODE,"name", queryterm2);
							...
							for (int i=0; i<query.size(); i++) {
								...
								for (int j=i+1; j<query.size(); j++) {
									...
									Node temp = graphDB_en.findNode(Labels.SINGLE_NODE,"name", query.get(i).toString().toLowerCase());
									Node temp2 = graphDB_en.findNode(Labels.SINGLE_NODE,"name", query.get(j).toString().toLowerCase());
						...
						for(Iterator i=query.iterator(); i.hasNext(); ) {
							...
							Node firstsourcenode = graphDB_en.findNode(Labels.SINGLE_NODE,"name", curQueryTerm.toLowerCase());
							...
							//	LOGGER.info("Activating: "+ sourcenodename);
								...
								Node temp = graphDB_en.findNode(Labels.SINGLE_NODE,"name", sourcenodename.toLowerCase());
								Iterable<Relationship> allRelationships = temp.getRelationships();
							    for (Relationship relationship : allRelationships) 
							    {
						   ...
						   LOGGER.info("Centroid candidates: " +centroidcandidates.size() + "  " + centroidcandidates.toString());
							...
							for (int i=0; i<centroidcandidates.size(); i++) {
								...
								if (nodedistances.containsKey(candidate)) {
									...
									if (((Double)nodedistances.get(candidate)).doubleValue()<Double.MAX_VALUE) {
										
										double curdistance = ((Double)nodedistances.get(candidate)).doubleValue();
										double curaveragepathlength = curdistance / query.size();
										centroidcandidatesdata.put(candidate, curaveragepathlength);
									}
								...
								Node n1 = graphDB_en.findNode(Labels.SINGLE_NODE,"name", candidate.toLowerCase());
								
								for (int j=0; j<query.size(); j++) {
									
									String curQueryTerm = query.get(j).toString();
									
									Node n2 = graphDB_en.findNode(Labels.SINGLE_NODE,"name", curQueryTerm.toLowerCase());
				...
				LOGGER.info("<-getCentroidbySpreadingActivation() ended...");
