...
public class CrawlController extends Configurable {
  ...
  //static final Logger logger = LoggerFactory.getLogger(CrawlController.class);
	...
	private final static Logger logger = Logger.getLogger(CrawlController.class.getName());
  ...
  protected boolean seemsfinished;
  ...
  protected Crawler currentcrawler;

  public CrawlController(CrawlConfig config, PageFetcher pageFetcher, RobotstxtServer robotstxtServer, Crawler thiscrawler) throws Exception {
    super(config);
            ...
            synchronized (waitingLock) {
              ...
              while (true) {
                ...
                if (!someoneIsWorking) {
                 ...
                 //   logger.info("vor  frontier.close();");
                    
                    
                    frontier.close();
                    
                //    logger.info("vor  docIdServer.close();");
                    
                    docIdServer.close();
                    
                //    logger.info("vor  pageFetcher.shutDown2();");
                    
                //    pageFetcher.shutDown2();
                    
                //    logger.info("vor  pageFetcher.shutDown();");
                    
                    pageFetcher.shutDown();
                    
                //    logger.info("vor  finished true");
  ...
  public void addSeed(String pageUrl) {
	  ...
	  logger.info("Addseed called in class");;
  ...
  public void addSeed(String pageUrl, int docId) {
	  logger.info("Addseed2 called in class");;
    String canonicalUrl = URLCanonicalizer.getCanonicalURL(pageUrl);
    logger.info("canonicalUrl: " +canonicalUrl);;
    if (canonicalUrl == null) {
    	logger.info("Invalid seed URL: {}");
    } else {
      if (docId < 0) {
        ...
        try {
          docIdServer.addUrlAndDocId(canonicalUrl, docId);
        	...
        	logger.info("Could not add seed: {}"+e.getMessage());
        }
      }
      
    //  logger.info("Vor weburl");;
      ...
      if (!robotstxtServer.allows(webUrl)) {
    	  logger.info("Robots.txt does not allow this seed: {}" + pageUrl); // using the WARN level here, as the user specifically asked to add this seed
      } else {
    	//  logger.info("vor frontier");;
        frontier.schedule(webUrl);
       // logger.info("nach frontier");;
      }
    }
  }
