...
public class Frontier extends Configurable {
  ...
  public void schedule(WebURL url) {
	 // System.out.println("Frontier Schedule");
    int maxPagesToFetch = config.getMaxPagesToFetch();
  //  System.out.println("Frontier nach getmaxpagestofetch " +maxPagesToFetch);
   // synchronized (mutex) {
      try {
    	  
        if (maxPagesToFetch < 0 || scheduledPages < maxPagesToFetch) {
       // 	 System.out.println("vor work queus");
          workQueues.put(url);
        //  System.out.println("Nach work queus");
          scheduledPages++;
          counters.increment(Counters.ReservedCounterNames.SCHEDULED_PAGES);
        //  System.out.println("Nach counters increment");
        }
      } catch (DatabaseException e) {
       logger.warn("Error while putting the url in the work queue" + e);
      }
   // }
    //  System.out.println("Nach Frontier Schedule");
  }
